...
namespace Hm2Flac3D
{
    ...
    internal class Flac3dCommandWriters
    {
        ...
        /// <summary> 所有的命令文件所在的文件夹的绝对路径 </summary>
        ...
        public StreamWriter GetWriter(ElementType elementType, string writerName, string fileSuffix = null)
        {
            ...
            {
                string filePath = Path.Combine(CommandDirectory, fileName);
                //
        ...
        {
            string filePath = Path.Combine(CommandDirectory, FileMain + FileSuffixSel);
            FileStream fs = new FileStream(filePath, FileMode.OpenOrCreate, FileAccess.Write);
            StreamWriter sw = new StreamWriter(fs);
            //
            sw.WriteLine(
...
");
            ...
            if (names.Contains(FileZone, StringComparer.OrdinalIgnoreCase))
            {
                sw.WriteLine($"im {FileZone}");
                names.Remove(FileZone);
            }
            // 其他结构单元
